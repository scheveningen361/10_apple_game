...
//
// Usage examples:
//   go run ./cmd/apple-game                         → AnA vs AnA+B, 100 boards
//   go run ./cmd/apple-game -nia -n 100             → NIA vs AnA, 100 boards
//   go run ./cmd/apple-game -ana-only -n 1000       → AnA benchmark
//   go run ./cmd/apple-game -greedy-bench -n 100000 → Greedy large-scale benchmark
//   go run ./cmd/apple-game -solver                 → stdin board → AnA moves stdout (macro)
...
func main() {
	...
	perfectOut := flag.String("perfect-out", "data/generated/perfect_game.txt", "perfect game output file")
	...
	slOut := flag.String("sl-out", "data/generated/sl_data.bin", "output file for -gen-sl")
	flag.Parse()
