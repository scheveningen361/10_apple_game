#!/usr/bin/env python3
"""
Apple Game 1000 Distribution Analysis
Reads data/raw/games_1000.txt, extracts scores, and generates visualizations
"""
...
print("Parsing data/raw/games_1000.txt...")
try:
...
scores = np.array([int(m) for m in re.findall(pattern, content)], dtype=int)
...
ax1.hist(scores, bins=40, density=True, alpha=0.7, color='steelblue', edgecolor='black')
...
stats_text = f"""
STATISTICS SUMMARY (AnA, 1000 games)
{'='*40}
...
plt.tight_layout()
plt.savefig('reports/figures/ana_distribution_1000.png', dpi=150, bbox_inches='tight')
print("✓ Graph saved: reports/figures/ana_distribution_1000.png")

# Print summary
print("\n" + "="*50)
print("STATISTICS SUMMARY (AnA, 1000 games)")
print("="*50)
print(f"Mean           : {scores.mean():.4f}")
print(f"Median         : {np.median(scores):.4f}")
print(f"SD             : {scores.std():.4f}")
print(f"MAD            : {np.mean(np.abs(scores - np.mean(scores))):.4f}")
...
stat, p_value = stats.shapiro(scores)
print(f"Shapiro-Wilk   : p={p_value:.6e} {'(Normal)' if p_value > 0.05 else '(Not Normal)'}")
